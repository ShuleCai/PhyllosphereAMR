...
# Read ARG carrying contig (ACC) taxonomy data and exclude 'Strain' column
acc_tax <- read.csv("data/ARG_carrying_contig_taxonomy.csv")
acc_tax <- acc_tax %>% select(-Strain) %>% filter(Kingdom != "Eukaryota")
...
# Filter taxonomy data for ARG and MGE carrying contigs
amcc_tax <- acc_tax %>% filter(contig_name %in% amcc_table$contig_name)

# Format taxonomy columns by appending prefixes
amcc_tax$Kingdom <- ifelse(is.na(amcc_tax$Kingdom), NA, paste0("k__", amcc_tax$Kingdom))
amcc_tax$Phylum  <- ifelse(is.na(amcc_tax$Phylum),  NA, paste0("p__", amcc_tax$Phylum))
amcc_tax$Class   <- ifelse(is.na(amcc_tax$Class),   NA, paste0("c__", amcc_tax$Class))
amcc_tax$Order   <- ifelse(is.na(amcc_tax$Order),   NA, paste0("o__", amcc_tax$Order))
amcc_tax$Family  <- ifelse(is.na(amcc_tax$Family),  NA, paste0("f__", amcc_tax$Family))
amcc_tax$Genus   <- ifelse(is.na(amcc_tax$Genus),   NA, paste0("g__", amcc_tax$Genus))
amcc_tax$Species <- ifelse(is.na(amcc_tax$Species), NA, paste0("s__", amcc_tax$Species))

# Concatenate taxonomy columns into a single string
tmp_dat <- amcc_tax %>% 
  unite("taxonomy", 2:8, sep = ";", na.rm = TRUE, remove = FALSE) %>% 
  select(contig_name, taxonomy)

# Merge taxonomy data with TPM data
dat2 <- left_join(tmp_dat, acc_tpm, by = c("contig_name" = "Contig"))
