...
tmp_dat <- acc_tax %>% 
  unite("taxonomy", 2:ncol(acc_tax), sep = ";",
...
myobj_d_tax <- metacoder::filter_obs(myobj, "tax_data", !has_no_reads, drop_taxa = T)
...
obj_o$data$tax_abund <- calc_taxon_abund(obj_o, "tax_data",
                                         cols = groups$Sample)
