...
# Filter samples for non-agricultural environments
sample_non_agriculture <- (project_metadata %>% filter(Agricultural == "Non-agricultural"))$Sample

# Process non-agricultural data
acc_non_agri_union_tpm <- data.frame(contig_name = acc_tpm$Contig,
                                     with_MGE = acc_tpm$Contig %in% amcc_names$contig_name,
                                     abun_avg=rowMeans(acc_tpm[, sample_non_agriculture])) %>% 
  left_join(acc_tax %>% unique(), by = "contig_name") %>% 
  mutate(random_color = ifelse(with_MGE, "#ef8b70", "#fae2ae"),
         Genus_plot = paste0(Family, "_", 1:nrow(acc_tpm)),
         abun_avg_sqrt = abun_avg) %>% 
  filter(Kingdom=="Bacteria" & !is.na(Order) & abun_avg > 0)

# Generate a treemap visualization
pdf("figures/Fig2b.pdf", family="ArialMT", width=8, height = 3)
treemap(acc_non_agri_union_tpm,
