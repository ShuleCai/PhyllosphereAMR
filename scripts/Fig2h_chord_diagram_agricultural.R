...
both_df_taxo_abun <- read.csv("./data/Contig_co_occurrence_ARG_MGE.csv")

# Summarize agricultural abundance by ARG_group and MGE_major_category
# The summarise produces a table with columns: ARG_group, MGE_major_category, abun_agri
both_df_taxo_abun_agri <- both_df_taxo_abun %>% 
  # group rows by ARG_group and MGE_major_category and sum the 'Agricultural' values
  group_by(ARG_group, MGE_major_category) %>% 
  dplyr::summarise(abun_agri=sum(Agricultural)) %>% 
  # sort by descending abundance for inspection
  arrange(desc(abun_agri))
...
mat_agri <- both_df_taxo_abun_agri %>% 
...
# chordDiagram will draw links weighted by values in mat_agri
pdf("./figures/Fig2h.pdf", family = "ArialMT")
