...
for (i in seq_along(k_values)) {
  ...
  silhouette_avg <- mean(silhouette(kmeans_result$cluster, 
                                   dist(as.matrix(selected_data)))[, 3])
  silhouette_scores[i] <- silhouette_avg
  
  # Calculate Davies-Bouldin Score 
  davies_bouldin_scores[i] <- index.DB(as.matrix(selected_data), 
                                      cl = kmeans_result$cluster)$DB
  
  # Calculate Calinski-Harabasz Score 
  cluster_stats <- cluster.stats(d = dist(as.matrix(selected_data)), 
...
p2 <- ggplot(results, aes(x = k, y = Davies_Bouldin_Score)) +
  ...
  geom_point(color = "orange3") +
  geom_vline(xintercept = optimal_k, linetype = "dashed", color = "black") +
  labs(title = "Davies-Bouldin Score", 
       x = "Number of Clusters (K)", 
...
ggsave(combined_plot, 
       filename = "figures/kmeans_score.pdf",
       family = "ArialMT", 
       height = 4, width = 7)
...
ggplot(umap_df, aes(x = V1, y = V2, color = Cluster)) +
  geom_point() +
  labs(title = "UMAP Plot with K-means Clustering", 
       x = "UMAP Dimension 1", 
       y = "UMAP Dimension 2") +
  theme_base()

ggsave(filename = "figures/MAG_3co_cluster_umap.pdf", 
       family = "ArialMT", 
...
pdf("figures/MAG_3co_cluster_Parallel-Coordinates-Plot2.pdf", 
