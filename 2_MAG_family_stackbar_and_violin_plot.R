...
data_s <- data_s %>% mutate(Cluster = ifelse(Cluster == 3, "Class A",
                                        ifelse(Cluster == 2, "Class B", "Class C")))
write.csv(data_s, "MAG_bigtable_w_Clusters.csv", row.names = FALSE)
...
pdf("figures/Cluster_family_stackbar.pdf", family = "ArialMT", width = 5, height = 6)
...
# Save the percentage data for reference
write.csv(percentage_data_other, "figures/Cluster_family_stackbar.csv", row.names = FALSE)
...
pdf("figures/Cluster_ARG_MGE_VFG_violin.pdf", family = "ArialMT")
ggarrange(plot_ARG_count, plot_MGE_count, plot_VFG_count, legend,
          ncol = 1, nrow = 4, heights = c(1, 1, 1, 0.2))
dev.off()

# Save the data used for violin plots
write.csv(selected_data, "figures/Cluster_ARG_MGE_VFG_violin_data.csv", row.names = FALSE)
